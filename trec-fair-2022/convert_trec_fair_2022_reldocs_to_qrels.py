...
def load_doc_ids(path):
    ...
    with open(path, 'r') as f:
        ...
        assert len(docIDs) == trec_2022_fair_num_unique_docs
    return docIDs
...
trec_2022_fair_num_docs = 6475537
trec_2022_fair_num_unique_docs = 6475401
    ...
    for idx in tqdm(query['rel_docs'].keys()):
        ...
        for doc_id in line:
            if doc_id not in rel_docs:
                output = str(query['id'][idx]) + " 0 " + str(doc_id) + " 1"
                outf.write(output + '\n')
                rel_docs.add(doc_id)
        if args.random_negative_samples:
            pos_inds = get_pos_inds_from_docs(doc_ids, list(rel_docs))
            neg_inds = negsamp_vectorized_bsearch(pos_inds, trec_2022_fair_num_unique_docs, len(rel_docs))
            assert len(neg_inds) == len(rel_docs)
            neg_elements = [doc_ids[i] for i in neg_inds]
            for doc_id in neg_elements:
                output = str(query['id'][idx]) + " 0 " + str(doc_id) + " 0"
                outf.write(output + '\n')
                assert doc_id not in rel_docs
            #print(f"{query['id']2 {len(rel_docs)} {len(pos_inds)} {len(neg_elements)}")
        elif args.run_negative_samples:
            pos_inds = get_pos_inds_from_run(runs[query['id'][idx]], list(rel_docs))
            run_length = len(runs[query['id'][idx]])
            ...
            neg_elements = [runs[query['id'][idx]][i] for i in neg_inds]
            for doc_id in neg_elements:
                output = str(query['id'][idx]) + " 0 " + str(doc_id) + " 0"
                outf.write(output + '\n')
                assert doc_id not in rel_docs
